...
def test_write_falls_back_to_see_when_it_is_not_set():
    # Nenastavene `write` = totez co `see`; jinak by kazde okno potrebovalo
    # obe ACL a vetsina by je mela stejne.
    access = Access(see=Acl.of("user:hana"))
    assert access.for_verb(Verb.WRITE) == Acl.of("user:hana")


def test_write_set_separately_is_independent_of_see():
    # Verejne log okno, ktere smi vyprazdnit jen spravce.
    access = Access(see=Acl.of(PUBLIC), write=Acl.of("user:hana"))
    assert access.for_verb(Verb.SEE) == Acl.of(PUBLIC)
    assert access.for_verb(Verb.WRITE) == Acl.of("user:hana")


def test_unset_see_means_inherit_not_open():
    assert Access().for_verb(Verb.SEE) is None
...
def test_object_with_its_own_acl_does_not_inherit():
    chain = [Access(see=Acl.of("user:hana")), Access(see=Acl.of(PUBLIC))]
    assert effective_acl(Verb.SEE, chain, default=Acl.of(USERS)) == Acl.of("user:hana")


def test_object_without_acl_takes_the_one_from_its_screen():
    chain = [Access(), Access(see=Acl.of("group:ucetni"))]
    assert effective_acl(Verb.SEE, chain, default=Acl.of(USERS)) == Acl.of("group:ucetni")


def test_object_and_screen_without_acl_fall_to_the_instance_default():
    chain = [Access(), Access()]
    assert effective_acl(Verb.SEE, chain, default=Acl.of(USERS)) == Acl.of(USERS)


def test_inheritance_is_per_verb():
    # Okno zdedi `see` od plochy, ale `write` ma vlastni.
    chain = [Access(write=Acl.of("user:hana")), Access(see=Acl.of(PUBLIC))]
    assert effective_acl(Verb.SEE, chain, default=Acl.empty()) == Acl.of(PUBLIC)
    assert effective_acl(Verb.WRITE, chain, default=Acl.empty()) == Acl.of("user:hana")


def test_empty_chain_falls_to_the_default():
    assert effective_acl(Verb.SEE, [], default=Acl.of(USERS)) == Acl.of(USERS)


def test_an_explicitly_empty_acl_is_not_the_same_as_unset():
    # "Nikdo" je rozhodnuti a nesmi se prepsat dedicnosti na "kdokoli".
    chain = [Access(see=Acl.empty()), Access(see=Acl.of(PUBLIC))]
    assert effective_acl(Verb.SEE, chain, default=Acl.of(USERS)) == Acl.empty()
...
def test_step_up_lives_in_access_not_beside_it():
    ...
    assert Access(see=Acl.of(PUBLIC), step_up=True).step_up is True


def test_step_up_does_not_change_who_is_in_the_acl():
    # Krok navic se pta "jsi to fakt ty, ted" - neni to dalsi ACL.
    access = Access(see=Acl.of("user:hana"), step_up=True)
    assert access.for_verb(Verb.SEE) == Acl.of("user:hana")


def test_access_is_a_value():
    assert Access(see=Acl.of(PUBLIC), step_up=True) == Access(
        see=Acl.of(PUBLIC), step_up=True
    )
